function [temp,press,rho]=stdatmo(alt,Tzero,Units,GeomFlag)
...
if Tzero==288.15;
       Toffset=0;
elseif Tzero~=288.15;
       Toffset=-288.15+Tzero;
end

if nargin >= 3 && isstruct(Units)
    U = true;
    u = Units;
    if isa(alt,'DimensionedVariable')
        alt = alt/u.m;
    end
...
if nargin <= 2 && all(alt(:) <= 11000) %quick troposphere-only code
    TonTi=1-2.255769564462953e-005*alt;
